...
namespace 
{
	...
	void dbclose(PGconn* conn) {
		logger::log(LOGGER_SRC, "debug", std::format("closing database {:p}", static_cast<void*>(conn)));
		if (conn) 
			PQfinish(conn);
	}
	...
	struct dbutil 
	{
		...
		{
			logger::log(LOGGER_SRC, "debug", std::format("connecting to {} {:p}", name, static_cast<void*>(conn.get())));
